...
import pdb
# from ogb.lsc import WikiKG90Mv2Dataset, WikiKG90MEvaluator
...
class KGDatasetWiki(KGDataset):
    ...
    * test.txt stores the triples in the test set.
    ...
    The triples are stored as 'head_nid\trelation_id\ttail_nid'.
    '''
    def __init__(self, path, name='wikikg90m-v2'):
        self.name = name
        ...
        self.valid_dict['h,r->t']['t_candidate'] = np.load(osp.join(path, name, 'processed/val_candidate.npy'))
        self.test_dict = self.dataset.test_dict("test-challenge")
