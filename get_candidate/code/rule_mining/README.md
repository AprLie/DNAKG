# DNAKG-RuleMining
Here we provide the original code of AMIE and our code for generating candidates on WikiKG90Mv2 by using AMIE. 
The AMIE code is from https://github.com/lajus/amie.

step 1:
run ```python get_candidate/code/rule_mining/preprocess.py``` to generate sub-graphs of the data based on which rule-mining is performed. Outputs are saved in `./rules_input_file/`.
...
to generate the rules mined from the sub-graphs. Please save the output to directory `/rules_file`.
...
The README document of AIME is presented below.
# AMIE 
